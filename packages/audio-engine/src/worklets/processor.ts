...
interface Track {
  ...
  recordingSectionIndex?: number;
  recordingStartSample?: number;
}
...
// ─── Helpers ───────────────────────────────────────────────────────────────────
function getOrCreateSectionData(
  track: Track,
  sectionIndex: number,
  sectionLen: number,
): SectionData {
...
class LiveLooperProcessor extends AudioWorkletProcessor {
  ...
  private quantization: { snapToGrid: boolean; gridResolution: number } | null =
    null;
      ...
      switch (type) {
        ...
        case "CONFIG": {
          ...
          if (typeof smartSnapEnabled === "boolean") {
            ...
            console.log(
              `[AudioWorklet] CONFIG received. smartSnapEnabled=${smartSnapEnabled}`,
            );
          }
          if (quantization) {
        ...
        case "SET_BPM": {
          ...
          if (
            oldSamplesPerBeat > 0 &&
            Math.abs(newSamplesPerBeat - oldSamplesPerBeat) > 0.001
          ) {
            ...
            const currentSectionRelative =
              this.currentSample - this.sectionSampleOffset;
            const newSectionRelative = Math.floor(
              currentSectionRelative * ratio,
            );
        ...
        case "UNDO_LAYER": {
          ...
          // Propagate undo to linked downstream sections
          let currentIdx = this.currentSectionIndex;
          for (let i = currentIdx + 1; i < this.sections.length; i++) {
            const nextSec = this.sections.find((s) => s.index === i);
            if (nextSec && nextSec.trackLinks && nextSec.trackLinks[track.id]) {
              const nextSd = track.sections.get(i);
              if (nextSd && nextSd.layers.length > 0) {
                nextSd.layers.pop();
                rebuildMaster(nextSd);
              }
            } else {
              break; // chain broken
            }
        ...
        case "SET_BUFFER": {
          ...
          const rawBuf = new Float32Array(buffer);
          ...
          const sd = getOrCreateSectionData(track, sectionIndex, sectionLen);
        ...
        case "CONFIG_CHANNELS": {
          ...
          trackConfigs.forEach(
            (config: { trackId: number; mode: "mono" | "stereo" }) => {
  ...
  private commitLayerStandard(track: Track, sectionLen: number) {
    ...
    let currentIdx = recSectionIndex;
    for (let i = currentIdx + 1; i < this.sections.length; i++) {
      const nextSec = this.sections.find((s) => s.index === i);
      if (nextSec && nextSec.trackLinks && nextSec.trackLinks[track.id]) {
        ...
        if (nextLen > 0) {
          ...
          const adaptedBuf = adaptBufferToLength(layerBuf, nextLen);
          nextSd.layers.push({ buffer: adaptedBuf });
          for (let s = 0; s < nextLen; s++)
            nextSd.masterBuffer[s] += adaptedBuf[s];
        }
      } else {
        break; // chain broken
      }
  ...
  private stretchAllSections(newSamplesPerBeat: number, ratio: number) {
    for (const track of this.tracks) {
      track.sections.forEach((sd, sectionIndex) => {
        ...
        newMaster.set(
          stretchedMaster.subarray(
            0,
            Math.min(newSectionLen, stretchedMaster.length),
          ),
        );
        ...
        for (const layer of sd.layers) {
          ...
          newLayerBuf.set(
            stretchedLayer.subarray(
              0,
              Math.min(newSectionLen, stretchedLayer.length),
            ),
          );
  ...
  private commitLayerWithSmartSnap(track: Track, sectionLen: number) {
    ...
    for (let i = 0; i < onsets.length - 1; i++) {
      ...
      if (srcLength > 0 && tgtLength > 0) {
        ...
        const sourceSegment = compensatedBuffer.slice(
          Math.floor(startSrc),
          Math.floor(endSrc),
        );
    ...
    for (let s = 0; s < xfadeSamples; s++) {
      if (s < sectionLen) {
        ...
        stretchedBuffer[sectionLen - 1 - s] =
          stretchedBuffer[sectionLen - 1 - s] * fade;
      }
    }
    ...
    for (let i = currentIdx + 1; i < this.sections.length; i++) {
      ...
      if (nextSec && nextSec.trackLinks && nextSec.trackLinks[track.id]) {
        ...
        if (nextLen > 0) {
          const nextSd = getOrCreateSectionData(track, i, nextLen);
          const adaptedBuf = adaptBufferToLength(stretchedBuffer, nextLen);
          nextSd.layers.push({ buffer: adaptedBuf });
          for (let s = 0; s < nextLen; s++)
            nextSd.masterBuffer[s] += adaptedBuf[s];
        }
  ...
  process(
    ...
    const inputChannel =
      trackInputs[0] && trackInputs[0][0] ? trackInputs[0][0] : null;

    let sectionLen = this.currentSectionLen();
    ...
    for (let i = 0; i < outputCount; i++) {
      ...
      for (let t = 0; t < 4; t++) {
        ...
        if (tInput) {
          const val =
            tInput.length > 1 && this.tracks[t].channelMode === "stereo"
      ...
      if (this.smartSnapWindowSamples > 0) {
        ...
        this.inputHistoryWriteIdx =
          (this.inputHistoryWriteIdx + 1) % this.smartSnapWindowSamples;
      }
      ...
      if (this.rtlTestActive) {
        ...
        if (!this.rtlSpikeDetected && inputChannel && inputChannel[i] > 0.3) {
          ...
          this.port.postMessage({
            type: "RTL_MEASURED",
            samples: measured,
            latencyMs: (measured / this.sampleRate) * 1000,
          });
        }
      ...
      for (let t = 0; t < 4; t++) {
        ...
        if (
          !track.isMuted &&
          currentSection?.trackLinks[track.id] &&
          (track.state === "PLAYING" ||
            track.state === "ARMED" ||
      ...
      for (const track of this.tracks) {
        ...
        if (track.state === "ARMED" && sectionSample === 1) {
          track.state = "RECORDING";
          track.recordingSectionIndex = this.currentSectionIndex;
          track.recordingStartSample = this.currentSample - 1;
          ...
          for (let s = 0; s < this.smartSnapWindowSamples; s++) {
            const readIdx =
              (this.inputHistoryWriteIdx + s) % this.smartSnapWindowSamples;
        ...
        if (track.state === "RECORDING") {
          ...
          if (tInput) {
            ...
            const val =
              tInput.length > 1 && track.channelMode === "stereo"
                ? (tInput[0][i] + tInput[1][i]) * 0.5
                : tInput[0][i];
        ...
        if (track.state === "POST_ROLL") {
          ...
          if (tInput) {
            ...
            if (
              writeIdx >= 0 &&
              writeIdx < sd.recordBuffer.length &&
              inputChannel
            ) {
          ...
          if (track.postRollSamplesRemaining <= 0) {
            if (this.smartSnapEnabled) {
              this.commitLayerWithSmartSnap(track, recSectionLen);
            } else {
              this.commitLayerStandard(track, recSectionLen);
            }
          }
        }
      }
      ...
      // At the very last sample of the section loop, switch to queued section or next sequential section
      if (sectionSample === 0 && sectionRelative > 0) {
        const nextIndex =
          this.queuedSectionIndex ??
        ...
        if (nextIndex !== this.currentSectionIndex) {
          ...
          // Re-calculate sectionLen for the new section
          sectionLen = this.currentSectionLen();
          ...
          for (const track of this.tracks) {
            if (
              track.state === "ARMED" &&
              newSection &&
              !newSection.trackLinks[track.id]
            ) {
              track.state = "IDLE";
            }
          }
    ...
    if (this.currentSample % 1024 === 0 && this.samplesPerBeat > 0) {
      ...
      const inputLevels = this.inputSums.map((sum) =>
        Math.sqrt(sum / Math.max(1, this.inputCount)),
      );

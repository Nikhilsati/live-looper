...
export class PhaserEffect extends BaseEffect {
  ...
  private wireAllpassChain(stages: 2 | 4): void {
    ...
    try {
      this.input.disconnect(this.phaserFeedbackInput);
    } catch {
      /* first call */
    }
    try {
      this.phaserFeedbackInput.disconnect();
    } catch {
      /* first call */
    }
    try {
      this.phaserAp1.disconnect();
    } catch {
      /* first call */
    }
    try {
      this.phaserAp2.disconnect();
    } catch {
      /* first call */
    }
    try {
      this.phaserAp3.disconnect();
    } catch {
      /* first call */
    }
    try {
      this.phaserAp4.disconnect();
    } catch {
      /* first call */
    }
    try {
      this.phaserFeedback.disconnect();
    } catch {
      /* first call */
    }

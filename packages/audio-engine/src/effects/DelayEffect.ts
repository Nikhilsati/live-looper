...
export class DelayEffect extends BaseEffect {
    ...
    private rewireForMode(mode: 'mono' | 'pingpong'): void {
        ...
        try { this.input.disconnect(this.afterDelay); } catch { /* first call */ }
        try { this.input.disconnect(this.delay); } catch { /* not connected */ }
        try { this.input.disconnect(this.delayPanL); } catch { /* not connected */ }
    ...
    update(settings: DelaySettings, bpm: number): void {
        ...
        this.delayMix.gain.setTargetAtTime(settings.enabled ? settings.mix : 0, t, 0.02);

...
export const useLooperStore = create<LooperStore>((set, get) => ({
  ...
  fetchFXPresets: async () => {
    ...
    if (!seeded || presets.length === 0) {
      ...
      await presetService.savePreset(
        "Large Hall",
        "module",
        { mix: 0.4, size: 3.5, predelay: 20, damping: 0.4, enabled: true },
        "reverb",
      );
      await presetService.savePreset(
        "Small Room",
        "module",
        { mix: 0.2, size: 0.8, predelay: 5, damping: 0.8, enabled: true },
        "reverb",
      );
      await presetService.savePreset(
        "Endless Cave",
        "module",
        { mix: 0.8, size: 5.0, predelay: 50, damping: 0.1, enabled: true },
        "reverb",
      );
      ...
      await presetService.savePreset(
        "Heavy Distortion",
        "module",
        { amount: 0.8, tone: 0.7, enabled: true },
        "drive",
      );
      await presetService.savePreset(
        "Light Crunch",
        "module",
        { amount: 0.3, tone: 0.5, enabled: true },
        "drive",
      );
      await presetService.savePreset(
        "Tape Crunch",
        "module",
        { amount: 0.6, tone: 0.2, enabled: true },
        "drive",
      );
      ...
      await presetService.savePreset(
        "Vocal Doubler",
        "module",
        { rate: 0.5, depth: 0.2, mix: 0.5, voices: 2, enabled: true },
        "chorus",
      );
      await presetService.savePreset(
        "Deep Swirl",
        "module",
        { rate: 1.5, depth: 0.8, mix: 0.7, voices: 1, enabled: true },
        "chorus",
      );
      ...
      await presetService.savePreset(
        "Aggressive Squeeze",
        "module",
        { threshold: -30, ratio: 8, gain: 6 },
        "compressor",
      );
      await presetService.savePreset(
        "Subtle Polish",
        "module",
        { threshold: -15, ratio: 2, gain: 2 },
        "compressor",
      );
  ...
  carryForwardTrack: async (
    ...
    const { currentProject, loadProject } = get();
    if (!currentProject?.id) return;
    await projectService.carryForwardTrack(
      currentProject.id,
      trackIndex,
      fromSectionId,
      toSectionId,
  ...
  setMode: async (targetMode) => {
    const state = get();
    const { success, error, snapshot } = await modeController.transitionTo(
      targetMode,
      state,
    );
    if (success) {
  ...
  setTrackState: (trackId, state) => {
    const action = state.isRecording
      ? "record"
    ...
    if (state.isArmed !== undefined) {
      sessionRecorder.logEvent("ARM_TRACK", {
        trackId,
        isArmed: state.isArmed,
      });
    }
    if (state.isMuted !== undefined) {
      sessionRecorder.logEvent("MUTE_TRACK", {
        trackId,
    ...
    set((prev) => {
      ...
      if (newTracks[trackId])
        newTracks[trackId] = { ...newTracks[trackId], ...state };
  ...
  handleEngineEvent: (event: EngineEvent) => {
    ...
    switch (event.type) {
      ...
      case "RTL_TIMEOUT":
        ...
        uiAlert(
          "Latency Calibration Timed Out. Ensure Output is looped to Input.",
        );
        break;
      case "PROJECT_LOADED": {
        const { project, tracks, sections, layerCounts, waveformDataMap } =
          event.payload;
        ...
        const soloedIds = newTracks
          .map((t, i) => (t.isSoloed ? i : -1))
          .filter((i) => i !== -1);
        if (soloedIds.length > 0) {
          audioEngine.applySolo(
            soloedIds,
            newTracks.map((t) => t.isMuted),
          );
        }
  ...
  setSolo: (trackId) => {
    ...
    sessionRecorder.logEvent("SOLO_TRACK", {
      trackId,
      isSoloed: !tracks[trackId].isSoloed,
    });

    // Simply toggle this track's solo — other tracks are unaffected
    set((prev) => ({
      tracks: prev.tracks.map((t, i) =>
        i === trackId ? { ...t, isSoloed: !t.isSoloed } : t,
      ),
    }));

    const updatedTracks = get().tracks;
    const soloedIds = updatedTracks
      .map((t, i) => (t.isSoloed ? i : -1))
      .filter((i) => i !== -1);
    audioEngine.applySolo(
      soloedIds,
      updatedTracks.map((t) => t.isMuted),
    );
  ...
  if (state.sections !== prevState.sections) {
    ...
    if (projectId) {
      state.sections.forEach((section, i) => {
        ...
        if (
          section.lengthInBars !== prev?.lengthInBars ||

...
export const uiAlert = (message: string, title: string = 'Notice', options?: Partial<DialogOptions>) => {
    return useDialogStore.getState().pushDialog({ type: 'alert', message, title, ...options });
};

export const uiConfirm = (message: string, title: string = 'Confirm', options?: Partial<DialogOptions>) => {
    return useDialogStore.getState().pushDialog({ type: 'confirm', message, title, ...options });
};

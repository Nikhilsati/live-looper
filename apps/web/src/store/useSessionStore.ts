...
export const useSessionStore = create<SessionState>((set, get) => ({
  ...
  startRecording: async () => {
    ...
    const snapshot: FrozenProjectSnapshot = {
      ...
      bpm: looperStore.bpm,
      quantization: {
        snapToGrid: looperStore.smartSnapEnabled,
        gridResolution: 16,
      },
    };
  ...
  stopRecording: async () => {
    ...
    try {
      ...
      console.error("Failed to stop recording and save session:", err);
    } finally {
      set({
        isSessionRecording: false,
  ...
  replaySession: async (session) => {
    ...
    const soloedIds = session.projectSnapshot.tracks
      .map((t, i) => (t.isSoloed ? i : -1))
      .filter((i) => i !== -1);
    if (soloedIds.length > 0) {
      audioEngine.applySolo(
        soloedIds,
        session.projectSnapshot.tracks.map((t) => t.isMuted),
      );
    }

    audioEngine.updateLiveTrackFX(
      session.projectSnapshot.liveTrack.fx,
      session.projectSnapshot.bpm,
    );
    audioEngine.setLiveTrackMute(session.projectSnapshot.liveTrack.isMuted);
    audioEngine.setSmartSnapEnabled(
      session.projectSnapshot.quantization.snapToGrid,
    );
    ...
    session.events.forEach((event) => {
      setTimeout(() => {
        ...
        switch (event.type) {
          case "PLAY":
            if (!currentLooperStore.isPlaying)
              currentLooperStore.startPlayback();
            break;
          case "STOP":
            if (currentLooperStore.isPlaying) currentLooperStore.stopPlayback();
            break;
          case "ARM_TRACK":
            currentLooperStore.setTrackState(event.payload.trackId, {
          ...
          case "SOLO_TRACK":
            if (
              currentLooperStore.tracks[event.payload.trackId].isSoloed !==
              event.payload.isSoloed
            ) {
          ...
          case "MUTE_LIVE_TRACK":
            currentLooperStore.setLiveTrackState({
              isMuted: event.payload.isMuted,
            });
            break;
          case "CLEAR_TRACK":
  ...
  togglePlayback: async () => {
    ...
    try {
      ...
      console.error("Error in togglePlayback:", e);
      if (looperStore.isPlaying) {
        looperStore.stopPlayback();
      }
    }

...
const KV: React.FC<{ label: string; value: React.ReactNode; mono?: boolean }> = ({ label, value, mono }) => (
    <div style={{ display: 'flex', gap: 8, alignItems: 'baseline' }}>
        <span style={{ color: '#6b7280', fontSize: 11, minWidth: 130, flexShrink: 0 }}>{label}</span>
        <span style={{
...
const SectionBlock: React.FC<{ title: string; accent: string; children: React.ReactNode }> = ({ title, accent, children }) => (
    <div style={{
...
const PlayBtn: React.FC<PlayBtnProps> = ({ isPlaying, isLoading, disabled, onPlay, onStop }) => {
    if (isLoading) {
        return (
            <span style={{
                fontSize: 10,
...
const DualWaveformViewer: React.FC<DualWaveformViewerProps> = ({ blob, rawBlob, bpm, timeSignature }) => {
    const canvasRef = useRef<HTMLCanvasElement>(null);
    const [loading, setLoading] = useState(true);
    const [error, setError] = useState<string | null>(null);
    const [zoom, setZoom] = useState(1);
    const [amp, setAmp] = useState(1);
    const [audioData, setAudioData] = useState<{ raw: Float32Array | null, snapped: Float32Array | null } | null>(null);
    ...
    useEffect(() => {
        ...
        if (samplesPerQuarter > 0 && blob.lengthSamples > 0) {
            ...
            const totalThirtySecondNotes = Math.ceil((blob.lengthSamples / samplesPerQuarter) * 8);
    ...
    }, [audioData, zoom, amp, bpm, timeSignature, blob.sampleRate, blob.lengthSamples, loading]);

    return (
        <div style={{ padding: 12, background: 'rgba(0,0,0,0.2)', borderRadius: 8, marginTop: 4 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8, fontSize: 10, color: '#9ca3af' }}>
                <span style={{ fontWeight: 700 }}>Waveform Comparison</span>
                <div style={{ display: 'flex', gap: 12 }}>
                    {rawBlob && (
                        <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                            <span style={{ display: 'inline-block', width: 8, height: 8, background: '#f87171', borderRadius: '50%' }} />
                            Raw (Unquantized)
                        </span>
                    )}
                    <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        <span style={{ display: 'inline-block', width: 8, height: 8, background: '#34d399', borderRadius: '50%' }} />
                        Snapped
                    </span>
                </div>
            </div>

            <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 12, marginBottom: 4, width: 'max-content', marginLeft: 'auto' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'rgba(255,255,255,0.05)', borderRadius: 4, padding: '2px 4px' }}>
                    <span style={{ fontSize: 10, color: '#9ca3af', marginRight: 4 }}>Gain:</span>
                    <button
                        disabled={amp <= 0.5}
                        onClick={() => setAmp(a => Math.max(0.5, a - 0.5))}
                        style={{ background: 'transparent', border: 'none', color: amp > 0.5 ? '#d1d5db' : '#4b5563', cursor: amp > 0.5 ? 'pointer' : 'default', padding: '0 4px', fontSize: 12 }}
                    >-</button>
                    <span style={{ width: 30, textAlign: 'center', fontSize: 10, color: '#9ca3af', lineHeight: '18px' }}>{amp}x</span>
                    <button
                        disabled={amp >= 10}
                        onClick={() => setAmp(a => Math.min(10, a + 0.5))}
                        style={{ background: 'transparent', border: 'none', color: amp < 10 ? '#d1d5db' : '#4b5563', cursor: amp < 10 ? 'pointer' : 'default', padding: '0 4px', fontSize: 12 }}
                    >+</button>
                </div>

                <div style={{ display: 'flex', alignItems: 'center', gap: 4, background: 'rgba(255,255,255,0.05)', borderRadius: 4, padding: '2px 4px' }}>
                    <span style={{ fontSize: 10, color: '#9ca3af', marginRight: 4 }}>Zoom:</span>
                    <button
                        disabled={zoom <= 1}
                        onClick={() => setZoom(z => Math.max(1, z - 1))}
                        style={{ background: 'transparent', border: 'none', color: zoom > 1 ? '#d1d5db' : '#4b5563', cursor: zoom > 1 ? 'pointer' : 'default', padding: '0 4px', fontSize: 12 }}
                    >-</button>
                    <span style={{ width: 30, textAlign: 'center', fontSize: 10, color: '#9ca3af', lineHeight: '18px' }}>{zoom}x</span>
                    <button
                        disabled={zoom >= 10}
                        onClick={() => setZoom(z => Math.min(10, z + 1))}
                        style={{ background: 'transparent', border: 'none', color: zoom < 10 ? '#d1d5db' : '#4b5563', cursor: zoom < 10 ? 'pointer' : 'default', padding: '0 4px', fontSize: 12 }}
                    >+</button>
                </div>
            </div>

            <div style={{ position: 'relative', height: 100, width: '100%', background: 'rgba(255,255,255,0.03)', borderRadius: 4, overflowX: 'auto', overflowY: 'hidden' }}>
                {loading && (
                    <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6b7280', zIndex: 10 }}>
                ...
                <div style={{ width: 800 * zoom, height: '100%' }}>
                    <canvas
                        ...
                        width={800 * zoom}
                        height={100}
                        style={{ width: '100%', height: '100%', display: 'block', opacity: loading ? 0.3 : 1 }}
                    />
                </div>
            </div>
        </div>
...
export const DevInspector: React.FC = () => {
    const { currentProject, tracks: storeTracksState, sections, bpm, mode, showDevInspector } = useLooperStore();
    ...
    const playLayer = useCallback(async (playId: string, blob: AudioBlobRecord) => {
    ...
    const playTrack = useCallback(async (trackId: string, layers: LayerRecord[]) => {
        stopCurrent();
        setLoadingTrackId(trackId);
        try {
            const trackBlobs = snap?.audioBlobs.filter(b =>
            ...
            // Decode all buffers
            const buffers = await Promise.all(trackBlobs.map(async b => {
            ...
            if (sources.length > 0) {
                sources[0].onended = () => {
                    if (activeSourcesRef.current.length > 0 && activeSourcesRef.current[0] === sources[0]) {
                        setPlayingTrackId(null);
                    }
                };
            }
    ...
    const totalBlobSize = snap?.audioBlobs.reduce((acc, b) => acc + b.lengthSamples * 2, 0) ?? 0;
                ...
                <div style={{ position: 'relative', zIndex: 1 }}>
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                            <span style={{ color: '#6366f1', fontSize: 13, fontWeight: 700 }}>◈ LIVE LOOPER · DEV INSPECTOR</span>
                    ...
                    {!currentProject ? (
                        ...
                        <div style={{ color: '#6b7280', padding: 20, textAlign: 'center' }}>
                            {loading ? 'Loading IndexedDB snapshot…' : 'Click Refresh to load snapshot.'}
                        </div>
                    ) : (
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                            ...
                            <div>
                                <SectionBlock title="📁 Project · IndexedDB" accent="#6366f1">
                                    ...
                                    <KV label="timeSignature" value={snap.project?.timeSignature} />
                                    <KV label="schemaVersion" value={snap.project?.schemaVersion} />
                                    <KV label="appVersion" value={snap.project?.appVersion} />
                                    <KV label="smartSnapEnabled" value={snap.project?.settings?.smartSnapEnabled ? 'true' : 'false'} />
                                    <KV label="createdAt" value={snap.project ? fmtDate(snap.project.createdAt) : undefined} />
                                    <KV label="updatedAt" value={snap.project ? fmtDate(snap.project.updatedAt) : undefined} />
                                    <KV label="masterLengthSamples" value={snap.project?.masterLengthSamples} />
                                </SectionBlock>
                            ...
                            {/* ── Right Column ── */}
                            <div>
                                <SectionBlock title={`🎛  Tracks · ${snap.tracks.length}`} accent="#10b981">
                                    {snap.tracks.map((t) => {
                                        const trackLayers = snap.layers.filter(l => l.trackId === t.id);
                                        const activeLayers = trackLayers.filter(l => !l.deletedAt);
                                        return (
                                            <div key={t.id} style={{
                                                padding: '8px 10px',
                                                ...
                                                <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4 }}>
                                                    ...
                                                    <span style={{ color: '#f9fafb', fontWeight: 700, fontSize: 12 }}>{t.name}</span>
                                                    ...
                                                    <div style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 8 }}>
                                                        <PlayBtn
                                                            ...
                                                            disabled={(!!playingLayerId || (!!playingTrackId && playingTrackId !== t.id)) || activeLayers.length === 0}
                                                            onPlay={() => playTrack(t.id, activeLayers)}
                                                            onStop={stopCurrent}
                                                        />
                                                        <span style={{ color: '#6b7280', fontSize: 10 }}>
                                                            {trackLayers.length} layer{trackLayers.length !== 1 ? 's' : ''}
                                                        </span>
                                                    </div>
                                                </div>
                                                <div style={{ color: '#6b7280', fontSize: 10 }}>id: {shortId(t.id)}</div>
                                            </div>
                                        );
                                    })}
                                </SectionBlock>

                                <SectionBlock title={`📐 Sections · ${snap.sections.length}`} accent="#3b82f6">
                                    {snap.sections.map((s) => {
                                        const sectionLayers = snap.layers.filter(l => l.sectionId === s.id);
                                        return (
                                            <div key={s.id} style={{
                                                padding: '8px 10px',
                                                background: 'rgba(255,255,255,0.03)',
                                                ...
                                                <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 4 }}>
                                                    <span style={{ color: '#f9fafb', fontWeight: 700 }}>{s.name}</span>
                                                    <Tag color="#60a5fa">{s.lengthInBars ?? '?'} bars</Tag>
                                                    <span style={{ color: '#6b7280', fontSize: 10, marginLeft: 'auto' }}>
                                                        {sectionLayers.length} layer{sectionLayers.length !== 1 ? 's' : ''}
                                                    </span>
                                                </div>
                                                <KV label="lengthSamples" value={s.lengthSamples > 0 ? s.lengthSamples.toLocaleString() : 'not set'} />
                                                <div style={{ color: '#6b7280', fontSize: 10 }}>id: {shortId(s.id)}</div>
                                            </div>
                                        );
                                    })}
                                </SectionBlock>
                            ...
                            <div style={{ gridColumn: '1 / -1' }}>
                                ...
                                >
                                    {snap.layers.length === 0
                                        ? <span style={{ color: '#6b7280' }}>No layers recorded yet.</span>
                                        : (
                                            <div style={{ overflowX: 'auto' }}>
                                                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                                                    <thead>
                                                        <tr style={{ color: '#6b7280', textAlign: 'left', borderBottom: '1px solid rgba(255,255,255,0.07)' }}>
                                                    ...
                                                    <tbody>
                                                        {snap.layers.map((layer) => {
                                                            const blob = snap.audioBlobs.find(b => b.id === layer.audioBlobId);
                                                            const rawBlob = layer.rawAudioBlobId ? snap.audioBlobs.find(b => b.id === layer.rawAudioBlobId) : undefined;
                                                            const track = snap.tracks.find(t => t.id === layer.trackId);
                                                            const section = snap.sections.find(s => s.id === layer.sectionId);
                                                            const estimatedSize = (blob ? blob.lengthSamples * 2 : 0) + (rawBlob ? rawBlob.lengthSamples * 2 : 0);
                                                            const isThisPlaying = playingLayerId === layer.id;
                                                            const isThisPlayingRaw = playingLayerId === `${layer.id}-raw`;
                                                            const isThisLoading = loadingLayerId === layer.id;
                                                            const isThisLoadingRaw = loadingLayerId === `${layer.id}-raw`;
                                                            const isAnotherPlaying = (!!playingLayerId && !isThisPlaying && !isThisPlayingRaw) || !!playingTrackId;
                                                                ...
                                                                <React.Fragment key={layer.id}>
                                                                    <tr
                                                                        style={{
                                                                            ...
                                                                            color: '#d1d5db',
                                                                            background: (isThisPlaying || isThisPlayingRaw)
                                                                    ...
                                                                    >
                                                                        ...
                                                                        <td style={{ padding: '5px 8px', display: 'flex', gap: 4 }}>
                                                                            <PlayBtn
                                                                                ...
                                                                                onPlay={() => blob && playLayer(layer.id, blob)}
                                                                                onStop={stopCurrent}
                                                                            />
                                                                            {rawBlob && (
                                                                                <div style={{ marginLeft: 4 }} title="Play Raw (Unsnapped)">
                                                                                    <PlayBtn
                                                                                        isPlaying={isThisPlayingRaw}
                                                                                        isLoading={isThisLoadingRaw}
                                                                                        disabled={isAnotherPlaying || !rawBlob}
                                                                                        onPlay={() => rawBlob && playLayer(`${layer.id}-raw`, rawBlob)}
                                                                                        onStop={stopCurrent}
                                                                                    />
                                                                                </div>
                                                                            )}
                                                                        ...
                                                                        <td style={{ padding: '5px 8px', color: '#a78bfa', fontFamily: 'monospace' }}>
                                                                            {shortId(layer.id)}
                                                                            {layer.deletedAt && (
                                                                                <div style={{ marginTop: 2 }}><Tag color="#ef4444">UNDO-ED</Tag></div>
                                                                            )}
                                                                        </td>
                                                                        <td style={{ padding: '5px 8px' }}>
                                                                            <span style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                                                                                {track && (
                                                                                    <span style={{ width: 8, height: 8, borderRadius: '50%', background: track.color, display: 'inline-block' }} />
                                                                                )}
                                                                        ...
                                                                        <td style={{ padding: '5px 8px', color: '#6b7280' }}>{layer.order}</td>
                                                                        <td style={{ padding: '5px 8px' }}>{layer.gain.toFixed(2)}</td>
                                                                        <td style={{ padding: '5px 8px', color: '#7c3aed', fontFamily: 'monospace' }}>
                                                                            <div>{blob ? shortId(blob.id) : <span style={{ color: '#ef4444' }}>missing!</span>}</div>
                                                                            {layer.rawAudioBlobId && (
                                                                                <div style={{ marginTop: 2, color: '#f59e0b' }} title="Raw Audio Blob">
                                                                                    {rawBlob ? shortId(rawBlob.id) : <span style={{ color: '#ef4444' }}>missing raw!</span>}
                                                                                </div>
                                                                            )}
                                                                        </td>
                                                                        ...
                                                                        <td style={{ padding: '5px 8px', color: '#34d399' }}>
                                                                            {blob ? sec(blob.lengthSamples, blob.sampleRate) : '—'}
                                                                        </td>
                                                                        ...
                                                                        <td style={{ padding: '5px 8px' }}>
                                                                            <div style={{ display: 'flex', gap: 6 }}>
                                                                                <button
                                                                                    ...
                                                                                    style={{
                                                                                        background: expandedLayerId === layer.id ? 'rgba(99,102,241,0.2)' : 'rgba(255,255,255,0.05)',
                                                                                        border: expandedLayerId === layer.id ? '1px solid rgba(99,102,241,0.4)' : '1px solid rgba(255,255,255,0.1)',
                                                                                        borderRadius: 4,
                                                                                        color: expandedLayerId === layer.id ? '#a5b4fc' : '#d1d5db',
                                                                                        fontSize: 10,
                                                                                        padding: '2px 6px',
                                                                                        cursor: 'pointer',
                                                                                    }}
                                                                                >
                                                                                    {expandedLayerId === layer.id ? 'Hide View' : 'View'}
                                                                                </button>
                                                                                <button
                                                                                    onClick={async () => {
                                                                    ...
                                                                    {expandedLayerId === layer.id && blob && (
                                                                        <tr>
                                                                            <td colSpan={12} style={{ padding: '0 8px 8px 8px', borderBottom: '1px solid rgba(255,255,255,0.04)' }}>
                                                                                <DualWaveformViewer
                                                                                    blob={blob}
                                                                                    rawBlob={rawBlob}
                                                                                    bpm={snap.project?.bpm || bpm}
                                                                                    timeSignature={snap.project?.timeSignature || '4/4'}
                                                                                />
                                                                            </td>
                                                                        </tr>
                                                                    )}
                    ...
                    <div style={{
                        ...
                        live-looper-db · IndexedDB · auto-refreshes every 3s while open · layer playback via Web Audio API
                    </div>
                </div>
            </div>
        </div>
